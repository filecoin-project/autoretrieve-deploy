...
describe("SaveRetrievalEvent", () => {
  it("saves a retrieval event", () => {
    ...
    useCase.execute({ retrievalEvent });

    sinon.assert.calledOnce(retrievalEventRepo.save);
    sinon.assert.calledWith(retrievalEventRepo.save, retrievalEvent);
  });
});